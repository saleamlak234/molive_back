...
// Create a manual commission for a user (admin only)
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const sourceUser = sourceUserId
        ? await User.findById(sourceUserId)
        : null;
      if (sourceUserId && !sourceUser) {
        return res
          .status(404)
          .json({ message: "Selected source user not found" });
      }

      const prevBalance = user.balance || 0;
      const newBalance = prevBalance + Number(amount);

      const commissionDescription = sourceUser
        ? `Manual ${type === "dailyReferral" ? "daily referral" : type === "directReferral" ? "direct referral" : type} commission from level ${level} child ${sourceUser.fullName}`
        : description.trim();

      // Create commission record
      const commission = await Commission.create({
        user: user._id,
        fromUser: sourceUser ? sourceUser._id : undefined,
      ...
      // Update user's balance and totalCommissions
      await User.findByIdAndUpdate(user._id, {
        $inc: { balance: Number(amount), totalCommissions: Number(amount) },
      });
      ...
      const transaction = new Transaction({
        ...
        description: `Manual commission (${type}) credited to user`,
        sourceTransaction: commission._id,
      ...
      if (user.telegramChatId) {
        await telegramService.sendMessage(
          user.telegramChatId,
          `💰 Admin added a commission of ${Number(amount).toLocaleString()} ETB to your account. Reason: ${commissionDescription}`,
        );
      }

      res.json({ commission, transaction });
    } catch (error) {
...
// Get downline users at a specific level for the selected admin user
router.get(
  "/users/:userId/downline",
  checkAdminPermission("admin"),
  async (req, res) => {
    try {
      ...
      for (let i = 1; i <= level; i += 1) {
        downlineUsers = await User.find({ referredBy: { $in: currentUserIds } })
          .select("fullName referredBy")
          .lean();

        if (downlineUsers.length === 0) break;
        currentUserIds = downlineUsers.map((user) => user._id);
      }

      res.json({ users: downlineUsers });
    } catch (error) {
      console.error("Get downline users error:", error);
      res.status(500).json({ message: "Server error fetching downline users" });
    }
  },
);
